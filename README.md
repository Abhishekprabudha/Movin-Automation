...
Use the included Colab notebook first to compress the source screen recording into a small `source_video.zip`, then upload that small file to GitHub and run the workflow to create the final Ryan Neural narrated video. The workflow now exports the MP3 directly from the selected Microsoft neural voice without post-slowdown processing, so the delivered narration keeps the requested Ryan Neural voice quality.
...
- `scripts/build_video.py` — extracts/transcribes narration or uses the curated script, creates a Ryan Neural MP3, paces the video to that audio, and exports final MP4.
...
4. Keep defaults to fit the narration to 150 seconds and generate the delivered MP3 directly with Ryan Neural:
...
- `final_movin_ryan_neural.mp4` — final paced video aligned to the Ryan Neural narration
- `narration_en_gb_ryan.mp3` — Ryan Neural narration audio generated directly from the selected voice
