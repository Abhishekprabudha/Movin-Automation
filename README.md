...
- `.github/workflows/build-video.yml` — GitHub Actions workflow to generate the final 2x-slowed MP4 with a nice neural voice.
- `scripts/build_video.py` — extracts/transcribes narration or uses the curated script, creates a neural voice MP3, slows the narration by the requested factor, paces the video to that slowed audio, and exports the final MP4.
...
`SPEED_TO_SECONDS = 0` preserves the original source timing before upload. This is recommended because the GitHub workflow handles the narration, final 2x slowdown, and final video pacing.
...
4. Keep defaults to generate a smooth neural voice and slow both the voice narration and final video by 2x:
...
With these defaults, the narration is first fitted around the 150-second target and then slowed by 2x, so the final MP4 is approximately 300 seconds while the full video and voice narration remain synchronized.
...
- `final_movin_nice_neural_voice.mp4` — final video paced to the 2x-slowed neural narration
- `narration_nice_neural_voice.mp3` — delivered neural narration audio slowed by 2x
