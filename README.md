...
Use the included Colab notebook first to compress the source screen recording into a small `source_video.zip`, then upload that small file to GitHub and run the workflow to create the final 6-minute Ryan Neural narrated video.
...
`SPEED_TO_SECONDS = 0` preserves the original narration timing. This is recommended because the GitHub workflow can then transcribe and revoice the same narration in `en-GB-RyanNeural` and pace the final output to 6 minutes.

Use `SPEED_TO_SECONDS = 360` only when you want Colab to also create a short visual preview. For source transcription, keep it at `0`.
...
4. Keep defaults for a 6-minute executive cut:
...
- `final_movin_ryan_neural.mp4` — final paced 6-minute video
